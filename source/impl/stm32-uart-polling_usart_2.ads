...
package STM32.UART.Polling_USART_2 is
   ...
   procedure Configure
     ...
     with Pre =>
       ...
       RX in (PA, 3) | (PD, 6);
   --  Configure USART_2 on given pins and baud rate

   procedure Set_Baud_Rate (Rate : Interfaces.Unsigned_32);
   --  Reconfigure USART_2 baud rate. Don't call while send in not complete
...
end STM32.UART.Polling_USART_2;
