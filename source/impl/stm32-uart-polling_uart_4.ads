...
package STM32.UART.Polling_UART_4 is
   ...
   procedure Configure
     ...
     with Pre =>
       ...
       RX in (PA, 1) | (PC, 11);
   --  Configure UART_4 on given pins and baud rate

   procedure Set_Baud_Rate (Rate : Interfaces.Unsigned_32);
   --  Reconfigure UART_4 baud rate. Don't call while send in not complete
