...
#### Generic timer with a single compare/capture channel (1 GPIO pin)

> TIM10, TIM11, TIM13, TIM14

Generic polling timer packages are the simplest general-purpose timer variant
(for example TIM10, TIM11, TIM13, TIM14). They provide one
compare/capture channel (channel 1 only) that can work in one of two
directions:

- Capture (Input): an external signal edge is sampled and the current counter
  value is latched into capture register. This value means
  measured time/position of the selected edge relative to the timer counter.
- Compare (Output): the counter is continuously compared with Compare value. When the
  counter reaches it, an output action is applied according to the selected
  compare mode. In this direction, Compare value is the compare threshold; in PWM modes
  it defines the duty boundary.

For Compare (Output), supported modes are:
Frozen, Active_On_Match, Inactive_On_Match, Toggle_On_Match,
Force_Inactive, Force_Active, PWM_Active, PWM_Inactive.

An extra compare/capture event can be generated by software with
Generate_Event (Compare_Capture => True).

The pin argument in Initialize is usually used for practical I/O and is
typically required for Capture (Input), because input capture needs the timer
channel signal routed from a GPIO pin.

Use a timer-specific polling package when you need PWM generation without
interrupt or DMA support. The following example configures TIM13 channel 1 on
PA6 to blink an LED with 2 second period and 25% duty cycle.

```ada
with STM32.Timer;
with STM32.Timer.Polling_TIM_13;

procedure Polling_PWM_TIM13 is
  package TIM renames STM32.Timer.Polling_TIM_13;
begin
  TIM.Initialize ((1 => (STM32.PA, 6)));  --  TIM13_CH1 on PA6

  TIM.Set_Frequency (2_000);          --  2 kHz counter clock
  TIM.Set_Auto_Reload_Value (4_000);  --  2 second PWM period
  TIM.Set_Compare_Value (1_000);      --  25% duty cycle (0.5 second on)

  TIM.Setup
   (Channel =>
     (Is_Input             => False,
      Active_Level          => 1,
      Compare_Mode          => STM32.Timer.PWM_Active,
      Compare_Value_Preload => True,
      Fast_PWM              => False));

  TIM.Configure
    (Setting => (Enable => STM32.Timer.True, others => <>));

  TIM.Generate_Event (Update => True);

  loop
    delay 1.0;
  end loop;
end Polling_PWM_TIM13;
```

#### Generic timer with 2 compare/capture channels

> TIM9, TIM12

These timers provide 2 compare/capture channels. They also support a new
trigger event in addition to update and compare/capture events, which can
be generated with `Generate_Event (Trigger => True)`.

Additionally, they support slave configuration where the timer's operation
is controlled by another timer or an external signal. The `Setup`
procedure is used to configure this behavior.
The following slave modes are available:
- `Reset` - A rising edge of the selected trigger input reinitializes the
...
- `Trigger` - The counter starts on a rising edge of the trigger (but it
  is not reset). Only the start of the counter is controlled.
- `External_Clock` - Rising edges of the selected trigger clock the counter.

The trigger input specifies the source of the trigger signal:
- `Other_Timer` - Internal trigger from another timer connected to this timer.
- `Edge_Detected` - TI1 edge detector.
- `Filtered_Timer_Input` - Filtered timer input 1 or 2 (TI1FP1 or TI2FP2).

`Trigger_Input` has two distinct uses in `TIM9` and `TIM12`:

- As the timer trigger source in `Setup (Slave_Mode => ..., Trigger_Input => ...)`.
  In this role it controls the slave-mode controller.
- As the input source for a capture/compare channel configured for input
  capture with `Use_Trigger_Input => True`. In this role the channel captures
  events from `TRC` instead of directly from `TI1` or `TI2`.

This means a single `Trigger_Input` selection can both control the timer
through `Slave_Mode` and feed capture channel(s) that use `TRC`.

Here is an example of Master/Slave timer configuration in Gated mode:

```ada
begin
   package Slave renames STM32.Timer.Polling_TIM_12;
   package Master renames STM32.Timer.Polling_TIM_13;
begin
   Master.Initialize ((1 => (STM32.PA, 6)));  --  TIM_13_CH1 on PA6

   Master.Set_Frequency (2_000);          --  2 kHz counter clock
   Master.Set_Auto_Reload_Value (399);    --  0.2 second PWM period (5Hz)
   Master.Set_Compare_Value (99);         --  25% duty cycle (0.5 second on)
   ...
   Slave.Initialize ((1 => (STM32.PB, 14)));  --  TIM12_CH1 on PB14

   Slave.Set_Frequency (2_000);          --  2 kHz counter clock
   Slave.Set_Auto_Reload_Value (3);  --  500 Hz second PWM period
   Slave.Set_Compare_Value (1, 2);   --  50% duty cycle (0.5 second on)

   Slave.Setup
     (Slave_Mode    => STM32.Timer.Gated,
      Trigger_Input => (Kind => STM32.Timer.Other_Timer, Timer => 13),
      Channels      =>
        (1 =>
          (Is_Input              => False,
           Active_Level          => 1,
